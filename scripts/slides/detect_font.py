...
def _build_fc_synonym_map_fclist() -> dict[str, set[str]]:
    """Build synonym map via ``fc-list`` (Linux, or any platform with fontconfig)."""
    proc = subprocess.run(
        [
            "fc-list",
            "--format",
...
def _build_fc_synonym_map_windows() -> dict[str, set[str]]:
    """Build synonym map from the Windows registry font list."""
    import winreg
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, reg_path) as key:
            ...
            while True:
                try:
                    display_name, _, _ = winreg.EnumValue(key, idx)
                except OSError:
                    break
                idx += 1
                # display_name looks like "Arial (TrueType)" or "Noto Sans CJK SC (TrueType)"
                clean = re.sub(r"\s*\([^)]*\)\s*$", "", display_name).strip()
                if not clean:
                    continue
                norm = normalize_font_family_name(clean)
                no_space = norm.replace(" ", "")
                names = {norm, no_space}
                for n in list(names):
                    bucket = syn.setdefault(n, set())
                    bucket.update(names)
    except OSError:
        pass
    return syn
...
def _build_fc_synonym_map_macos() -> dict[str, set[str]]:
    """Build synonym map via ``system_profiler SPFontsDataType`` (macOS)."""
    syn: dict[str, set[str]] = {}
    try:
        proc = subprocess.run(
        ...
        for font_entry in data.get("SPFontsDataType", []):
            family = font_entry.get("family", "")
            full_name = font_entry.get("full_name", "")
            ps_name = font_entry.get("postscriptname", "")
            names: set[str] = set()
            for raw in (family, full_name, ps_name):
                if not raw:
                    continue
                norm = normalize_font_family_name(raw)
                if norm:
                    names.add(norm)
                    names.add(norm.replace(" ", ""))
            for n in list(names):
...
def _build_fc_synonym_map() -> dict[str, set[str]]:
    """Build synonym map from system fonts; dispatches to platform-specific backends.

    Priority: fc-list (cross-platform if fontconfig installed) → Windows registry → macOS
    system_profiler. Returns an empty dict if no backend is available.
    """
    # Try fc-list first — works on Linux and any platform with fontconfig installed.
    if shutil.which("fc-list"):
        try:
            return _build_fc_synonym_map_fclist()
        except (subprocess.SubprocessError, OSError):
            pass

    system = _platform.system()
    if system == "Windows":
        result = _build_fc_synonym_map_windows()
        if result:
            return result
    elif system == "Darwin":
        result = _build_fc_synonym_map_macos()
        if result:
            return result

    # Fallback: empty map (font checks will report everything as missing).
    return {}
...
def _find_soffice() -> str:
    """Locate the ``soffice`` binary across Windows, macOS, and Linux."""
    ...
    if system == "Windows":
        ...
        candidates.extend(["/usr/bin/soffice", "/usr/bin/libreoffice"])
    for c in candidates:
        if os.path.isfile(c):
            return c
    return "soffice"  # last resort — let subprocess raise if truly absent
